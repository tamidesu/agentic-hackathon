...
#: ЕДИНЫЙ СЛОВАРЬ КАТЕГОРИЙ — общий для шага 5 (извлечение ковенантов),
#: шага 10 (категоризация транзакций) и этого реестра.
#:
#: Это главный стык проекта. Если ковенант просит `capital_expenditure`,
#: а реестр размечен как `capex`, агрегат окажется пустым, actual — нулём,
#: а статус, скорее всего, неверным. Словарь объявлен один раз и
#: импортируется обеими сторонами; расхождение ловится тестом.
CATEGORIES: tuple[str, ...] = (
    ...
    "lease",             # арендные платежи
    "insurance",
    #: МАРКЕТИНГ — ОТДЕЛЬНАЯ СТАТЬЯ, А НЕ ЧАСТЬ opex.
    #:
    #: Выяснено измерением. Без этой статьи 129 строк из 157 в `opex`
    #: оказывались маркетинговыми — 291 млн из 339, — и операционные
    #: расходы раздувались в разы. У P1 `opex` выходил 19.27 млн вместо
    #: 4.00 млн, и ковенант «капиталоёмкость» показывал 0.09 при
    #: настоящих 0.46: вердикт переворачивался.
    #:
    #: Проверка: opex P1 без маркетинга даёт 4 001 430.98 при требуемых
    #: по ключу 4 004 362 — расхождение 0.07%. На всём наборе выделение
    #: статьи подняло долю верных вердиктов с 25 до 29 из 36.
    "marketing",         # страховые премии
    "financing_inflow",  # поступления по финансированию
    "other",             # не относится ни к одной статье ковенантов
)

#: Псевдокатегория «любая статья» — для ковенантов, где важен получатель,
#: а не статья расхода (платежи связанным сторонам).
ANY_CATEGORY = "any"

#: Признаки контрагента, влияющие на попадание в агрегат.
PARTY_KINDS: tuple[str, ...] = ("related", "unrestricted_subsidiary")

#: Охват показателя.
SCOPES: tuple[str, ...] = ("borrower", "group")
...
OBSERVED_FORMS: dict[str, dict[str, Any]] = {
    ...
    "conditional": {
        ...
        # EBITDA не статья реестра, а разность двух статей: категории
        # "ebitda" в словаре нет и быть не должно. Раньше здесь стояло
        # AGG("ebitda") — как few-shot материал это учило модель выдумывать
        # статью, дающую ПУСТУЮ сумму и ложный COMPLIANT с actual=0.
        "дерево": {"op": "DIV", "args": [
            {"op": "AGG", "category": "financing_inflow"},
            {"op": "SUB", "args": [{"op": "AGG", "category": "revenue"},
                                   {"op": "AGG", "category": "opex"}]}]},
